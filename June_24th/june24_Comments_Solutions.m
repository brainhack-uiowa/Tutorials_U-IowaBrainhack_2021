...
% Next we introduced the ins and outs of basic data structures (June11).

% Last week we combined this information to discuss flow control (June 18th). Together,
...
% Let us load a dataset: The one below is built into matlab and will serve as
% our example
...
load census1994.mat % The file we shared has some of the variables made below. 
% The core pieces are: adultdata,adulttest, and Description
...
size(adultdata,2) % Should say 15 -- Check this by clicking on 'adultdata' in the workspace to verify
...
% 1. How to check if adultdata is a table?
...
% For more problems, it seems mathworks supports more ways to test your
% knowledge and understanding :). We recommend tackling a few a day,
% everyday to practice your skills! 

% https://www.mathworks.com/matlabcentral/cody/problems?term=group%3A%22Cody+Challenge%22&sort=created+asc
...
class(adultdata) % Class Solution June 24.
...
% Question 2, Class Solution June 24.

variable = 0
count = 0
for i = 1:height(adultdata);
    if table2array(adultdata(i,10)) == 'Female';
        variable = variable + table2array(adultdata(i,11));
        count = count + 1;
    end
end
...
% Question 5, Class solution June 24th
histogram(table2array(adultdata(:,9)))
xlabel('Race'), ylabel('Racial Frequency'), title('Racial Disparity')
